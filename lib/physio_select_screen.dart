...
const List<PriceTier> kPriceTiers = [
  PriceTier(label: '₹600 - ₹700',     sublabel: 'Entry Level Therapists',  icon: Icons.person_outline_rounded, min: 600,    max: 700),
...
class PhysioScreen extends StatefulWidget {
  ...
  const PhysioScreen({super.key, this.preSelectedServiceName});
...
class _PhysioScreenState extends State<PhysioScreen>
  ...
  void _startBookingWithService(PhysioService service) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        image: DecorationImage(
          image: (ApiService.cachedDynamicInnerBannerMap?['Physio'] != null && ApiService.cachedDynamicInnerBannerMap!['Physio']!.isNotEmpty)
              ? NetworkImage(ApiService.getMediaUrl(ApiService.cachedDynamicInnerBannerMap!['Physio']!)) as ImageProvider
...
class _ServicesTab extends StatelessWidget {
  final VoidCallback onDemo;
  final VoidCallback onEnquire;
  final void Function(PhysioService) onSelectService;
  const _ServicesTab({
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              ...[
                ('🎓', 'Certified Physiotherapist with 5-10+ years experience'),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          // Right-corner arrow button — pre-selects this plan and opens
          // the booking flow (user can still add more services inside).
          GestureDetector(
            onTap: onSelect,
...
class _BookingFlowSheet extends StatefulWidget {
  ...
  final PhysioService? initialService;
...
class _BookingFlowSheetState extends State<_BookingFlowSheet> {
  ...
  final Set<PhysioService> _selectedServices = {};
  ...
  void _toggleService(PhysioService s) {
    setState(() {
  ...
  void _submitBookingToBackend({
    ...
    required PhysioService primary,
...
class _StepService extends StatelessWidget {
  final Set<PhysioService> selected;
  final void Function(PhysioService) onToggle;
  const _StepService(
  ...
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Choose Physio Plan(s)',
            style: TextStyle(
...
class _StepAddress extends StatelessWidget {
  final List<PhysioService> selectedServices;
      ...
      children: [
        ...
        Row(
          ...
          children: [
            const Expanded(
              child: Text(
                  'Please provide the pricing range for physio services.',
                  style: TextStyle(
...
class _StepDemoConfirm extends StatelessWidget {
  final List<PhysioService> services;
  final DateTime date;
